...
def parse_finance_page(ticker):
	"""
	Grab financial data from SGX page
	...
	for retries in range(5):
		try:	
			url = "https://www2.sgx.com/securities/equities/%s"%(ticker)
			...
			xpath_key_stock_table = '//div[contains(@class,"Overview")]//div[contains(@class,"table-table")]/div'
			xpath_open_price = '//b[contains(text(),"Previous Open Price:")]/following-sibling::span/text()'
			...
			sgx_data = {
							...
							"ticker":ticker,
							"url":url,
							"Previous open price":open_price,
							"open_date":open_date,
